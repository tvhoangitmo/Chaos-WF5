...
def embed_lsb(image_path, secret_message, output_path="../images/output/lsb_method/stego_lsb.png", x0=0.5, r=3.9):
    """
    Embed secret message using LSB steganography with chaotic encryption
    ...
    total_pixels = cover_image.shape[0] * cover_image.shape[1] * cover_image.shape[2]
    if message_length > total_pixels:
        raise ValueError(f"Message too large. Need {message_length} bits, but image can only hold {total_pixels} bits")
    
    # Create stego image
    stego_image = cover_image.copy()
    
    # Embed message length first (32 bits)
    length_binary = np.array(list(np.binary_repr(message_length, width=32)), dtype=np.uint8)
    
    # Embed length and encrypted message
    binary_data = np.concatenate([length_binary, encrypted_message])
...
def extract_lsb(stego_image_path, x0=0.5, r=3.9):
    """
    Extract secret message from stego image using LSB steganography with chaotic decryption
    ...
    length_bits = []
    for i in range(32):
        length_bits.append(flat_image[i] & 1)
    ...
    for i in range(32, 32 + message_length):
        message_bits.append(flat_image[i] & 1)
    
    # Decrypt and convert bits to text
    encrypted_bits = np.array(message_bits)
...
def main():
    """Demo function for LSB steganography with chaotic encryption"""
    cover_image = "../images/input/cover_1.jpg"
    # secret_message = "This is a secret message hidden using LSB steganography with chaotic encryption!"
    ...
    print("=== LSB Steganography with Chaotic Encryption Demo ===")
    ...
    # Embed message
    try:
        stego_path = embed_lsb(cover_image, secret_message, x0=x0, r=r)
        print(f"Stego image saved: {stego_path}")
        
        # Extract message
        extracted_message = extract_lsb(stego_path, x0=x0, r=r)
